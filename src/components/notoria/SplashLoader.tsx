...
const SplashLoader = ({ isLoading }: SplashLoaderProps) => {
  const [phase, setPhase] = useState<'icon' | 'loader'>('icon');

  useEffect(() => {
    if (isLoading) {
      setPhase('icon');
      const timer = setTimeout(() => setPhase('loader'), 1200);
  ...
  return (
    <AnimatePresence>
      {isLoading && phase === 'icon' && (
        ...
        >
          <motion.div
            className="flex items-center justify-center rounded-[2.5rem] bg-black shadow-2xl"
        ...
        >
          ...
          <div className="relative flex flex-col items-center gap-8">
            ...
            >
              ...
              />
              <motion.div className="absolute inset-[-10px] rounded-full bg-background" />
              <motion.div
